      ...
      <div class="flex flex-col min-h-screen">
        ...
        <div class="flex-1 flex flex-col max-w-screen-2xl w-full mx-auto px-4 sm:px-6 py-4 gap-3">
          @if (loadingEvents()) {
            ...
            <!-- Legend -->
            <div class="flex items-center gap-2 shrink-0 text-xs text-gray-500 dark:text-zinc-400">
...
export class FltReservation implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getEquipment().subscribe({
      next: (res) => {
        if (res.success) this.availableEquipment.set(res.equipment ?? []);
  ...
  addToBasket(): void {
    ...
    this.basket.update(b => {
      ...
      return [...without, { date: day, startTime: startStr, endTime: endStr }].sort((a, b) => a.date.localeCompare(b.date));
    });
