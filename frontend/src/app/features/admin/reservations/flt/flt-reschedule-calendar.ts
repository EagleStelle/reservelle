            ...
            <div class="flex-1 grid grid-cols-7 overflow-auto" [style.grid-template-rows]="calendarRows()">
              @for (cell of calendarCells(); track $index) {
                <div
                  ...
                  [class.hover:bg-sky-50]="cell.day !== null && !cell.isPast && !cell.isToday"
                  ...
                  @if (cell.day !== null) {
                    ...
                    @if (cell.events.length > 0) {
                      <ul class="flex flex-col gap-0.5 overflow-hidden">
                        @for (ev of cell.events.slice(0, 3); track ev.organization + ev.startTime) {
                          <li
                          ...
                          >
                            ...
                            >{{ ev.eventKind === 'COORDINATION' ? '📋 Coordination' : ev.organization }}</span>
                          </li>
                        }
          ...
          <div class="shrink-0 border-t border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg px-4 sm:px-6 py-3">
            <div class="max-w-screen-2xl mx-auto flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <button type="button" (click)="save()"
                [disabled]="saving()"
                class="flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-white hover:bg-primary/90 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shrink-0">
                @if (saving()) {
        ...
        <div class="flex-1 overflow-auto max-w-screen-md mx-auto w-full px-4 sm:px-6 py-6 flex flex-col gap-4">
                  ...
                  >
                    <div class="flex-1 min-w-0">
                      <p
                        ...
                        [class.text-amber-700]="ev.eventKind === 'COORDINATION'"
                        [class.dark:text-amber-300]="ev.eventKind === 'COORDINATION'"
                      >{{ ev.eventKind === 'COORDINATION' ? '📋 Coordination Meeting' : ev.organization }}</p>
                      <p
                        class="text-[10px]"
          ...
          @if (basket().length > 0) {
            <div class="mt-1">
              ...
              <button type="button" (click)="save()"
                [disabled]="saving()"
                class="w-full flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2.5 text-sm font-medium text-white hover:bg-primary/90 transition-colors cursor-pointer shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
